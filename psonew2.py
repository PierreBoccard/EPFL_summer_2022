...
fitting_kwargs_list = [['update_settings', {'source_add_fixed' : [[1, ['n_max'] ,[int(sys.argv[1])]]]}],
                       ...
                       ['PSO', {'sigma_scale': 1., 'n_particles': 300, 'n_iterations': 230, 'threadCount': 32}]
                      ]
...
file = open(f'LLhood{sys.argv[3]}newbis.txt', 'w+')
result = str(llhood) 
file.write(result)
file.close()

file = open(f'bic{sys.argv[3]}newbis.txt', 'w+')
result = str(bic) 
file.write(result)
file.close()
...
for i in range(len(chain_list)):
    ...
    f.savefig(f'convergence{sys.argv[3]}newbis.jpg', dpi=150)
plt.close()
...
file = open(f"kwargs{sys.argv[3]}newbis.txt", "w+")
...
f.savefig(f'mainplot{sys.argv[3]}newbis.jpg', dpi=150)
plt.close()

f, axes = lensPlot.plot_separate()
f.savefig(f'separate{sys.argv[3]}newbis.jpg', dpi=150)
plt.close()

f, axes = lensPlot.plot_subtract_from_data_all()
f.savefig(f'subtract{sys.argv[3]}newbis.jpg', dpi=150)
plt.close()
...
fitting_kwargs_list = [['update_settings', {'lens_remove_fixed' : [[0, ['gamma']]]}],
                       ['MCMC', {'n_burn': 200, 'n_run': 400, 'walkerRatio' : 20, 'sigma_scale': 0.2, 'threadCount': 32}] #'walkerRatio' : 10 et enlever n_waklers, nburn = 0 
                      ]
...
file = open(f'LLhood{sys.argv[3]}new2bis.txt', 'w+')
result2 = str(llhood2) 
file.write(result2)
file.close()

file = open(f'bic{sys.argv[3]}new2bis.txt', 'w+')
result2 = str(bic2) 
file.write(result2)
file.close()
...
for i in range(len(chain_list2)):
    ...
    f.savefig(f'convergence{sys.argv[3]}new2bis.jpg', dpi=150)
plt.close()
...
file = open(f"kwargs{sys.argv[3]}new2bis.txt", "w+")
...
f.savefig(f'mainplot{sys.argv[3]}new2bis.jpg', dpi=150)
plt.close()

f, axes = lensPlot2.plot_separate()
f.savefig(f'separate{sys.argv[3]}new2bis.jpg', dpi=150)
plt.close()

f, axes = lensPlot2.plot_subtract_from_data_all()
f.savefig(f'subtract{sys.argv[3]}new2bis.jpg', dpi=150)
plt.close()
...
import pickle
with open('chain_list{sys.argv[3]}bis.pkl', 'wb') as file:
...
plot.savefig(f'corner1{sys.argv[3]}bis.jpg',dpi=150)
plt.close()
plot = corner.corner(samples_mcmc[:,8:], labels=param_mcmc[8:], show_titles=True, truths=param_array_truths[8:])
plot.savefig(f'corner2{sys.argv[3]}bis.jpg',dpi=150)
plt.close()
...
fig.savefig(f'mcmc{sys.argv[3]}bis.jpg', dpi=150)
plt.close()
